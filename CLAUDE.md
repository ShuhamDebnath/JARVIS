...
**Active phase:** ⏸ **Phase 1 — partially complete; P1.15 Action 2 deferred.** P1.1–P1.15 Actions 1 shipped (all Phase 1 YAML, contracts, crews, and FastAPI wiring on main). P1.15 Action 2 (live E2E on real LLMs) is **partially shipped** in commit `b882eb8` (2026-06-02): LLM routing swapped to direct MiniMax API (`minimax/MiniMax-M3` across all 12 agents), `MINIMAX_API_KEY` added to `env_validator.py REQUIRED_KEYS` + `.env.example`, 3 regressed tests fixed via new `env_valid_false` fixture in `tests/conftest.py`, and 2 latent YAML bugs fixed (literal JSON braces in `tasks.yaml` descriptions escaped to `{{...}}`). Pytest: **27/27 green**. The live E2E run itself is **blocked on a separate pre-existing bug**: CrewAI 0.86.0's `describe_field` (in `crewai/utilities/converter.py:245`) crashes with `AttributeError: 'types.UnionType' object has no attribute '__name__'` when a Pydantic contract (e.g. `ResearchInterpretation.ambiguity_flag`) uses `Optional[X]`, which evaluates to `X | None` (`types.UnionType`) under Python 3.11. Three patch options: (a) monkey-patch `describe_field` in `dept_crews.py` to handle `types.UnionType`; (b) refactor the contract to a sentinel string; (c) leave the contract as-is and accept the CrewAI incompatibility. **Awaiting user choice on (a)/(b)/(c) before re-running the E2E.** `scripts/run_live_e2e.py` (the runbook harness) is checked in and ready to be re-used once the bug is resolved.
